...
Page({
    ...
    data: {
        ...
        isTriggered: false, //用来标识下拉刷新是否被阻断
    },
    ...
    handleTimeUpdate(event) {
        //console.log(event, '监听视频播放');
        let videoTimeObj = { vid: event.currentTarget.id, currentTime: event.detail.currentTime }
        let { videoUpdateTime } = this.data
            //push之前考虑数组中有没有记录过这个视频
            /**
             * 思路整理：判断记录播放的时长的videoUpdateTime数组中是否有当前视频的播放记录
    ...
    async getVideoGroupListData() {
        ...
        console.log(this.data, 'this.ddddddd');
    ...
    handlePlay(event) {
         ...
         * 问题：解决多个视频可以同时播放的问题
         * 需求
         * 1.在点击播放的事件中，找到上一个播放的视频
         * 2.在播放新的视频之前，关闭上一个正在播放的视频
         * 关键：
         * 1.如何找到上一个视频的实例对象
         * 2.如何确认我们点击播放的视频和正在播放的视频不是同一个视频
         * 单例模式：
        ...
        this.videoContext = wx.createVideoContext(vid)
            //判断当前的视频之前是否播放过，是否有播放记录，如果有，那么久跳转至指定的播放位置
        let { videoUpdateTime } = this.data
    ...
    handleToLower() {
        ...
        //数据分页：1.后端分页 2.前端分页——因为网易没有开放分页的接口，所以我们这里做不了分页，只能模拟做
